...
Para especificar cada uno de estos requerimientos, así como su prioridad, se realizó una primera versión del Documento de Historias de Usuario (\texit{Product Backlog}). Éste fue modificándose a lo largo del desarrollo del proyecto. Para más información de la versión final del documento, ver apéndice~\ref{chap:Product Backlog}.
...
Por otra parte, en la figura~\ref{fig:diagramaClasesServidor} se puede observar la estructura de datos del servidor. En este caso, se tienen dos clases: \textit{user} y \textit{report}.
